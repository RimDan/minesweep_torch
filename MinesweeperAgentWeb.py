...
class MinesweeperAgentWeb(object):
    ...
    def get_loc(self):
        ...
        boards = {mode: pg.locateOnScreen(f'{IMGS}/{mode}.png', confidence=.9) for mode in modes.keys()}
    ...
    def get_action(self, state):
        board = torch.from_numpy(self.state.reshape(1, self.ntiles))
        ...
        if rand < self.epsilon: # random move (explore)
            ...
            moves = self.model(state.unsqueeze(0).to(device))
